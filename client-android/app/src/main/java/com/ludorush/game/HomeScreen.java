...
public final class HomeScreen extends BaseScreen {
    ...
    private View buildTopBar() {
        ...
        bar.addView(identity, new LinearLayout.LayoutParams(0, -1, 0.55f));

        bar.addView(currency("$", "18.7K", 0xffFFD426, true), new LinearLayout.LayoutParams(0, dp(44), 1.25f));
        addGap(bar, 4);
        bar.addView(currency("D", "1.3K", 0xff35D9FF, true), new LinearLayout.LayoutParams(0, dp(44), 1.15f));
        addGap(bar, 4);
    ...
    private View currency(String icon, String value, int color, boolean plus) {
        ...
        coin.setBackground(icon.equals("D")
                ? diamondDrawable()
                : circle(color, 0xffFF9D00, 2));
        ...
        if (plus) {
            ...
            add.setBackground(circle(0xff3CCC43, 0xffB6FF7E, 1));
    ...
    private View buildModes() {
        ...
        list.addView(modeRow("QUICK MATCH", "Find players online\nand play now!", 0xffD238F5, 0xff7010B8,
                "dice", () -> callback.startQuickMatch("classic_2p")), rowLp());
        list.addView(modeRow("1 ON 1", "Challenge a player\nin a duel", 0xffFFB60A, 0xffCA6800,
                "duel", () -> callback.startQuickMatch("classic_2p")), rowLp());
        list.addView(modeRow("4 PLAYER", "Classic Ludo with\n4 players", 0xff1DCF39, 0xff006A18,
                "four", () -> callback.startQuickMatch("classic_4p")), rowLp());
        list.addView(modeRow("PRIVATE ROOM", "Create a room and\ninvite your friends", 0xff159CFF, 0xff003EBD,
                "lock", () -> callback.navigateTo("lobby")), rowLp());
        list.addView(modeRow("PLAY OFFLINE", "Play against smart\nAI opponents", 0xffDB2CCB, 0xff701070,
                "bot", () -> callback.startBotMatch("classic_2p")), rowLp());
        list.addView(modeRow("TEAM UP", "Team up and win\ntogether", 0xffF14320, 0xff991000,
                "team", () -> callback.navigateTo("lobby")), rowLp());
        return list;
    }
    ...
    private View modeRow(String title, String sub, int start, int end, String art, Runnable click) {
        LinearLayout row = new LinearLayout(activity);
        row.setGravity(Gravity.CENTER_VERTICAL);
        row.setPadding(dp(10), dp(5), dp(9), dp(5));
        ...
        TextView desc = label(sub, 12, 0xffFFE0FF, BODY);
        desc.setMaxLines(2);
    ...
    private void addNav(LinearLayout nav, String label, String route, String icon, boolean active, boolean badge) {
        ...
        TextView text = label(label, active ? 12 : 11, Color.WHITE, DISPLAY);
        text.setGravity(Gravity.CENTER);
        text.setSingleLine(true);
        text.setShadowLayer(dp(2), 0, dp(2), 0xaa000000);
        item.addView(text, lp(-1, -2, 0, dp(2), 0, 0));
    ...
    private String displayName() {
        ...
        if (name == null || name.trim().isEmpty() || "Rush Tester".equals(name)) {
            return "Ibsam";
        }
        return name.trim();
    }
    ...
    private static final class CosmicParkBackdrop extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            for (int i = 0; i < 74; i++) {
                float x = ((i * 71 + 19) % 1000) / 1000f * w;
                float y = ((i * 127 + 43) % 1000) / 1000f * h;
                int color = new int[]{0xffFFFFFF, 0xffFFD426, 0xff32E9FF, 0xffFF4FD8, 0xff58FF3B}[i % 5];
                paint.setColor(color);
                if (i % 3 == 0) {
                    canvas.drawCircle(x, y, 2.2f + (i % 4), paint);
                } else {
                    canvas.save();
        ...
        private void drawWheel(Canvas canvas, float cx, float cy, float r) {
            paint.setStyle(Paint.Style.STROKE);
            paint.setStrokeWidth(r * 0.10f);
            paint.setColor(0xff26D7FF);
        ...
        private void drawCoaster(Canvas canvas, float w, float h) {
            paint.setStyle(Paint.Style.STROKE);
            paint.setStrokeWidth(w * 0.013f);
            paint.setColor(0xAAFF4FD8);
            path.reset();
        ...
        private void drawCoin(Canvas canvas, float cx, float cy, float r) {
            ...
            canvas.drawCircle(cx + r * 0.16f, cy + r * 0.20f, r, paint);
            paint.setColor(0xffFFD426);
            canvas.drawCircle(cx, cy, r, paint);
            paint.setStyle(Paint.Style.STROKE);
            paint.setStrokeWidth(r * 0.15f);
        ...
        private void drawGem(Canvas canvas, float cx, float cy, float r, int color) {
            ...
            canvas.drawPath(path, paint);
            paint.setColor(0x88FFFFFF);
            canvas.drawCircle(cx - r * 0.22f, cy - r * 0.22f, r * 0.22f, paint);
        }
    }
    ...
    private static final class AvatarView extends View {
        ...
        AvatarView(android.app.Activity activity) {
            super(activity);
        }
        ...
        protected void onDraw(Canvas canvas) {
            ...
            paint.setColor(0xffFFD426);
            canvas.drawCircle(cx, cy, r, paint);
            paint.setColor(0xff262066);
            canvas.drawCircle(cx, cy, r * 0.84f, paint);
            paint.setColor(0xffF5B16B);
            canvas.drawCircle(cx, cy + r * 0.08f, r * 0.43f, paint);
    ...
    private static final class LogoSceneView extends View {
        ...
        LogoSceneView(android.app.Activity activity, boolean dark) {
            super(activity);
            this.dark = dark;
        }
        ...
        private void drawCrown(Canvas canvas, float cx, float cy, float s) {
            path.reset();
            path.moveTo(cx - s, cy + s * 0.60f);
            path.lineTo(cx - s * 0.72f, cy - s * 0.28f);
            path.lineTo(cx - s * 0.28f, cy + s * 0.16f);
            path.lineTo(cx, cy - s * 0.64f);
            path.lineTo(cx + s * 0.28f, cy + s * 0.16f);
            path.lineTo(cx + s * 0.72f, cy - s * 0.28f);
            path.lineTo(cx + s, cy + s * 0.60f);
            path.close();
            paint.setShader(new LinearGradient(cx, cy - s, cx, cy + s,
                    0xffFFF49A, 0xffF49300, Shader.TileMode.CLAMP));
            canvas.drawPath(path, paint);
            paint.setShader(null);
            paint.setStyle(Paint.Style.STROKE);
            paint.setStrokeWidth(s * 0.08f);
            paint.setColor(0xffFFF2A1);
            canvas.drawPath(path, paint);
            paint.setStyle(Paint.Style.FILL);
        }
    ...
    private static final class LogoTextView extends View {
        ...
        LogoTextView(android.app.Activity activity) {
            super(activity);
        }
        ...
        protected void onDraw(Canvas canvas) {
            ...
            float ludoSize = Math.min(w * 0.19f, h * 0.31f);
            float rushSize = Math.min(w * 0.17f, h * 0.26f);
            float y1 = h * 0.53f;
            float y2 = h * 0.78f;

            paint.setShadowLayer(12f, 0, 9f, 0xdd000000);
            drawOutlinedLetters(canvas, "LUDO", w / 2f, y1, ludoSize, letterColors);

            paint.setTextSize(rushSize);
            paint.setStyle(Paint.Style.STROKE);
            paint.setStrokeWidth(Math.max(8f, rushSize * 0.16f));
            paint.setColor(0xff511074);
            canvas.drawText("RUSH", w / 2f, y2, paint);
            paint.setStyle(Paint.Style.FILL);
            paint.setShader(new LinearGradient(0, y2 - rushSize, 0, y2 + rushSize * 0.2f,
                    0xffffffff, 0xffF7B3FF, Shader.TileMode.CLAMP));
            canvas.drawText("RUSH", w / 2f, y2, paint);
            paint.setShader(null);
            paint.clearShadowLayer();

            paint.setColor(0xffFFD426);
            drawStar(canvas, w * 0.73f, y2 - rushSize * 0.14f, rushSize * 0.16f);
            paint.setStyle(Paint.Style.FILL);
        }

        private void drawOutlinedLetters(Canvas canvas, String text, float cx, float baseline, float textSize, int[] colors) {
            paint.setTextSize(textSize);
            float total = paint.measureText(text);
            float x = cx - total / 2f;
            for (int i = 0; i < text.length(); i++) {
                String letter = text.substring(i, i + 1);
                float letterWidth = paint.measureText(letter);
                float drawX = x + letterWidth / 2f;

                paint.setStyle(Paint.Style.STROKE);
                paint.setStrokeWidth(Math.max(7f, textSize * 0.10f));
                paint.setColor(0xfffff6cf);
                canvas.drawText(letter, drawX, baseline, paint);

                paint.setStyle(Paint.Style.FILL);
                paint.setShader(new LinearGradient(0, baseline - textSize, 0, baseline + textSize * 0.15f,
                        0xffffffff, colors[i % colors.length], Shader.TileMode.CLAMP));
                canvas.drawText(letter, drawX, baseline, paint);
                paint.setShader(null);
                x += letterWidth;
            }
        ...
        private void drawStar(Canvas canvas, float cx, float cy, float radius) {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                star.lineTo(
                        cx + (float) Math.cos(inner) * radius * 0.45f,
                        cy + (float) Math.sin(inner) * radius * 0.45f);
            }
            star.close();
    ...
    private static final class ModeIconView extends View {
        ...
        ModeIconView(android.app.Activity activity, String type) {
            super(activity);
            this.type = type;
        }
        ...
        protected void onDraw(Canvas canvas) {
            ...
            if ("dice".equals(type)) {
                drawDice(canvas, cx, cy, s * 0.78f, 5, 0xffFFFFFF);
            } else if ("lock".equals(type)) {
                drawLock(canvas, cx, cy, s * 0.78f);
            } else if ("bot".equals(type)) {
                drawBot(canvas, cx, cy, s * 0.78f);
            } else {
        ...
        private void drawBoard(Canvas canvas, float cx, float cy, float s) {
            ...
            canvas.drawRoundRect(b, s * 0.08f, s * 0.08f, paint);
            paint.setColor(0xffF32B2B);
            canvas.drawRect(b.left, b.top, cx, cy, paint);
            paint.setColor(0xffFFD426);
            canvas.drawRect(cx, b.top, b.right, cy, paint);
            paint.setColor(0xff1565E0);
            canvas.drawRect(b.left, cy, cx, b.bottom, paint);
            paint.setColor(0xff2DB34A);
            canvas.drawRect(cx, cy, b.right, b.bottom, paint);
            paint.setColor(0xffFFF6D0);
        ...
        private void drawPawn(Canvas canvas, float cx, float cy, float s, int color) {
            ...
            paint.setColor(0x88FFFFFF);
            canvas.drawCircle(cx - s * 0.13f, cy - s * 0.33f, s * 0.12f, paint);
        }
        ...
        private void drawVersus(Canvas canvas, float cx, float cy, float s) {
            ...
            paint.setTextSize(s);
            paint.setColor(0xffFFD426);
            paint.setShadowLayer(6f, 0, 4f, 0xaa000000);
            canvas.drawText("VS", cx, cy, paint);
            paint.clearShadowLayer();
        }
        ...
        private void drawLock(Canvas canvas, float cx, float cy, float s) {
            ...
            paint.setShader(new LinearGradient(cx, cy - s * 0.1f, cx, cy + s * 0.44f,
                    0xffFFF491, 0xffE58A00, Shader.TileMode.CLAMP));
            canvas.drawRoundRect(new RectF(cx - s * 0.36f, cy - s * 0.05f, cx + s * 0.36f, cy + s * 0.44f), s * 0.12f, s * 0.12f, paint);
            paint.setShader(null);
            paint.setColor(0xff5B2700);
        ...
        private void drawBot(Canvas canvas, float cx, float cy, float s) {
            ...
            paint.setShader(new LinearGradient(cx - s * 0.4f, cy - s * 0.3f, cx + s * 0.4f, cy + s * 0.35f,
                    0xffFFFFFF, 0xff8EDCFF, Shader.TileMode.CLAMP));
            canvas.drawRoundRect(new RectF(cx - s * 0.44f, cy - s * 0.29f, cx + s * 0.44f, cy + s * 0.34f), s * 0.22f, s * 0.22f, paint);
            paint.setShader(null);
            paint.setColor(0xff00356B);
            canvas.drawCircle(cx - s * 0.18f, cy, s * 0.09f, paint);
            canvas.drawCircle(cx + s * 0.18f, cy, s * 0.09f, paint);
            paint.setStyle(Paint.Style.STROKE);
    ...
    private static final class ArrowView extends View {
        ...
        ArrowView(android.app.Activity activity) {
            super(activity);
        }
        ...
        protected void onDraw(Canvas canvas) {
            ...
            paint.setColor(Color.WHITE);
            Path p = new Path();
            p.moveTo(w * 0.30f, h * 0.32f);
            p.lineTo(w * 0.70f, h * 0.50f);
            p.lineTo(w * 0.30f, h * 0.68f);
    ...
    private static final class NavIconView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            if ("home".equals(icon)) {
                Path p = new Path();
                p.moveTo(cx, h * 0.10f);
                p.lineTo(w * 0.86f, h * 0.45f);
                p.lineTo(w * 0.76f, h * 0.88f);
                p.lineTo(w * 0.24f, h * 0.88f);
                p.lineTo(w * 0.14f, h * 0.45f);
                p.close();
                canvas.drawPath(p, paint);
                ...
                paint.setStyle(Paint.Style.FILL);
                canvas.drawCircle(w * 0.43f, h * 0.82f, 4.8f, paint);
                canvas.drawCircle(w * 0.72f, h * 0.82f, 4.8f, paint);
